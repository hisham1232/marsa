                                                ...
                                                <table id="example24" class="display nowrap table table-hover table-striped table-bordered">
                                                    ...
                                                    <tbody>
                                                        <?php 
                                                            ...
                                                            if($appraisalType == 4) {
                                                                $rows = $helper->readData("SELECT at.id, at.requestNo, at.appraisal_type, at.staff_id, at.status, concat(s.firstName,' ',s.secondName,' ',s.thirdName,' ',s.lastName) as staffName, apd.name as appraisalType, sec.name as section FROM appraisal_technician as at LEFT OUTER JOIN section as sec ON sec.id = at.section_id LEFT OUTER JOIN appraisal_type_description as apd ON apd.id = at.appraisal_type LEFT OUTER JOIN staff as s ON at.staff_id = s.staffId WHERE at.section_id = $logged_in_section_id AND at.department_id = $logged_in_department_id"); //This is just for Technician Yet
                                                            } else if ($appraisalType == 5) {
                                                                $rows = $helper->readData("SELECT at.id, at.requestNo, at.appraisal_type, at.staff_id, at.status, concat(s.firstName,' ',s.secondName,' ',s.thirdName,' ',s.lastName) as staffName, apd.name as appraisalType, sec.name as section FROM appraisal_technician as at LEFT OUTER JOIN section as sec ON sec.id = at.section_id LEFT OUTER JOIN appraisal_type_description as apd ON apd.id = at.appraisal_type LEFT OUTER JOIN staff as s ON at.staff_id = s.staffId WHERE at.department_id = $logged_in_department_id"); //This is just for Technician Yet
                                                            }
                                                            foreach ($rows as $row) {
