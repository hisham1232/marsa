                        ...
                        <div class="container-fluid">
                            <div class="row page-titles">
                                <div class="col-md-5 col-8 align-self-center">
                                    <h3 class="text-themecolor m-b-0 m-t-0">All Approved E-Forms Request</h3>
                                    <ol class="breadcrumb">
                                ...
                                <div class="col-lg-12">
                                    <div class="card">
                                        <div class="card-header bg-light-yellow m-b-0">
                                            <h4 class="card-title">All Approved E-Forms Request
                                            <h6>كل طلبات الاستمارات الإلكترونية المعتمدة</h6>
                                        </div>
                                        <div class="card-body">
                                            <input type="hidden" class="updatedby" value="<?php echo $logged_name; ?>" />
                                            <div class="">
                                                <a class="btn btn-sm btn-dark" href="eforms_request_list.php?linkid=<?php echo $linkid; ?>">Pending</a> | <a class="btn btn-sm btn-info" href="eforms_request_list2.php?linkid=<?php echo $linkid; ?>"">Approved</a>
                                            </div>
                                                    ...
                                                    <tbody>
                                                        <?php
                                                            if(isset($_POST['searchByDate'])) {
                                                                ...
                                                                $SQL = "SELECT e.*, concat(s.firstName,' ',s.secondName,' ',s.thirdName,' ',s.lastName) as staffName, ef.name FROM e_forms_request as e LEFT OUTER JOIN staff as s ON e.requestBy = s.staffId LEFT OUTER JOIN e_forms as ef ON ef.id = e.eFormId FROM e_forms_request WHERE e.created >= '$startDate' AND e.created <= '$endDate' AND e.status = 'Approved' ORDER BY e.id DESC";
                                                            } else {
                                                                $SQL = "SELECT e.*, concat(s.firstName,' ',s.secondName,' ',s.thirdName,' ',s.lastName) as staffName, ef.name FROM e_forms_request as e LEFT OUTER JOIN staff as s ON e.requestBy = s.staffId LEFT OUTER JOIN e_forms as ef ON ef.id = e.eFormId WHERE e.status = 'Approved' ORDER BY e.id DESC";
                                                            }
                                                            //echo $SQL;
