                                                ...
                                                <table id="dynamicTable" class="display table-hover table-striped table-bordered"
                                                    ...
                                                    <tbody>
                                                        <?php
                                                            ...
                                                            $rows = $helper->readData("SELECT cas.*, c.requestNo, c.dateCreated, c.isCleared, concat(s.firstName,' ',s.secondName,' ',s.thirdName,' ',s.lastName) as staffName, d.name as department FROM clearance_approval_status as cas LEFT OUTER JOIN clearance as c ON cas.clearance_id = c.id LEFT OUTER JOIN staff as s ON cas.staffId = s.staffId LEFT OUTER JOIN employmentdetail as e ON c.staffId = e.staff_id LEFT OUTER JOIN department as d ON d.id = e.department_id WHERE cas.approverStaffId = '$staffId' AND current_flag = 1");
