                        ...
                        <div class="container-fluid">
                            <div class="row page-titles">
                                <div class="col-md-5 col-8 align-self-center">
                                    <h3 class="text-themecolor m-b-0 m-t-0">My Official Duty List</h3>
                                    <ol class="breadcrumb">
                                ...
                                <div class="col-lg-12"><!---start for list div-->
                                    <div class="card">
                                        <div class="card-body">
                                            ...
                                            ?>
                                            <h4 class="card-title">Official Duty List by [<?php echo trim($info['staffName']); ?> - <?php echo $info['department']; ?> - <?php echo $info['sponsor']; ?>]</h4>
                                            <h6 class="card-subtitle">قائمة طلبات مهام العمل الرسمي</h6>
                                                    ...
                                                    <tbody>
                                                        <?php 
                                                            ...
                                                            if(isset($_POST['searchByDate'])) {
                                                                ...
                                                                $rows = $data->readData("SELECT s.id, s.staff_id, s.requestNo, l.name as leave_type, s.dateFiled, s.startDate, s.endDate, s.total, s.modified, s.currentStatus FROM standardleave as s INNER JOIN leavetype as l ON s.leavetype_id = l.id AND s.staff_id = $staffId AND s.startDate >= '$startDate' AND s.endDate <= '$endDate' AND s.leavetype_id = 13 ORDER BY s.id DESC");
                                                            } else {
                                                                $rows = $data->readData("SELECT s.id, s.staff_id, s.requestNo, l.name as leave_type, s.dateFiled, s.startDate, s.endDate, s.total, s.modified, s.currentStatus FROM standardleave as s INNER JOIN leavetype as l ON s.leavetype_id = l.id AND s.staff_id = $staffId AND s.leavetype_id = 13");
                                                            }
                                                            $i=0;
                                                            if($data->totalCount != 0) {
                                                                foreach ($rows as $row) {
                                                                    ?>
                                                                    <tr>
                                                                        <td><a href="official_mission_my_details.php?id=<?php echo $row['requestNo']; ?>"><?php echo $row['requestNo']; ?></a></td>
