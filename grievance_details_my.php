<?php    
    ...
    if(!$helper->isUserLogged()) {
        ...
        $info = $check->singleReadFullQry("SELECT COUNT(id) as bilang FROM grievance WHERE id = $gid AND complainant = '$staffId'");
        $bilang = $info['bilang'];
        if($bilang > 0)
            $allowed =  true;
        else
                                        ...
                                        <div class="card-body bg-light-warning2">
                                            <?php 
                                                ...
                                                $sql = "SELECT h.*, j.name as jobtitle, concat(s.firstName,' ',s.secondName,' ',s.thirdName,' ',s.lastName) as contributor FROM grievance_history as h
                                                    ...
                                                    WHERE h.grievance_id = $gid AND e.isCurrent = 1";
                                            ...
                                            <div class="ribbon-wrapper card">
                                                ...
                                                <?php 
                                                    foreach($rows as $row) {
                                                        if($detail['meeting_date'] == '') {
                                                            $$meetingDeyt =  '-';
                                                        } else {
                                                            $meetingDeyt = date('M d, Y h:i:s A',strtotime($detail['meeting_date']));
                                                        }
                                                        ?>
                                                                    ...
                                                                    <div class="sl-right m-b-0">
                                                                        <div>
                                                                            ...
                                                                            <p class="m-b-0">Enter Date: <span class="font-weight-bold text-warning"><?php echo date('M d, Y h:i:s A',strtotime($row['dateEntered'])); ?></span></a> 
                                                                                &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;  Meeting Date: <span class="font-weight-bold text-warning"><?php echo $meetingDeyt; ?></span></p>
                                                                            <br/>
