<?php
    include "../../../classes/AjaxManipulators.php";
    $checkHistory = new AjaxManipulators;
    $getMainSHInfo = new AjaxManipulators;
    $id = $_POST['id'];
    $requestNo = $_POST['requestNo'];
    $info = $getMainSHInfo->singleReadFullQry("SELECT sh.staff_id, sh.currentStatus, sh.leaveDate, sh.currentStatus, sh.startTime, sh.endTime, sh.total, concat(s.firstName,' ',s.secondName,' ',s.thirdName,' ',s.lastName) as staffName, sh.currentStatus 
    FROM shortleave as sh
    LEFT OUTER JOIN staff as s ON s.staffId = sh.staff_id
    WHERE sh.id = $id");
    $currentStatus = $info['currentStatus'];
    $rows = $checkHistory->readData("SELECT sh.*, concat(s.firstName,' ',s.secondName,' ',s.thirdName,' ',s.lastName) as staffName FROM shortleave_history as sh
    LEFT OUTER JOIN staff as s ON s.staffId = sh.staff_id WHERE sh.shortleave_id = $id ORDER BY id DESC");
    if($checkHistory->totalCount != 0) {
        $i = 0;
        $new_approver = new AjaxManipulators;
        $table = '';
        $table .=    
                '<div class="d-flex flex-wrap">
                    <div>
                        <h3 class="text-dark">Staff: <span class="badge badge-default">'.$info['staff_id'].' - '.$info['staffName'].'</span></h3>
                    </div>
                    <div class="ml-auto">
                        <ul class="list-inline">
                            <li class="none">
                                <h3 class="text-dark">Request No: <span class="badge badge-primary">'.$requestNo.'</span></h3>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="d-flex flex-wrap">
                    <div>
                        <h3 class="text-dark">Leave Date: <span class="badge badge-default">'.date('F m, Y',strtotime($info['leaveDate'])).'</span></h3>
                    </div>
                    <div class="ml-auto">
                        <ul class="list-inline">
                            <li class="none">
                                <h3 class="text-dark">Duration: <span class="badge badge-default">'.$info['startTime'].' to '.$info['endTime'].'</span></h3>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="d-flex flex-wrap">
                    <div>
                        <h3 class="text-dark">Status: <span class="badge badge-default">'.$info['currentStatus'].'</span></h3>
                    </div>
                    <div class="ml-auto">
                        <ul class="list-inline">
                            <li class="none">
                                <h3 class="text-dark">No. of Hours: <span class="badge badge-default">'.$info['total'].'</span></h3>
                            </li>
                        </ul>
                    </div>
                </div>
                ';
        $table .='
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex flex-wrap">
                                <div>
                                    <h3 class="card-title">Leave Approval History</h3>
                                    <h6 class="card-subtitle">Arabic Text Here</h6>
                                </div>
                            </div>';
                            foreach($rows as $row){
                                $table .='
                                    <div class="ribbon-vwrapper card">
                                        <div class="ribbon ribbon-info ribbon-vertical-l">'.++$i.'</div>
                                        <h3 class="ribbon-content">'.$row["staffName"].'</h3>
                                        <p class="ribbon-content"><span class="text-muted">Status : <span class="text-primary">'.$row["status"].' on </span>
                                        <span class="text-info pull-right">'.date('F m, Y H:i:s',strtotime($row["created"])).'</span></span></p>
                                        <p class="ribbon-content"><i>'.$row["notes"].'</i></p>
                                    </div>    
                                    
                                ';
                            }
        $table .=
                        '</div>
                    </div>
                </div>
        ';        
        if($currentStatus == 'Pending') { //show only the type notes or comments if the status is Pending...
                $table.='
                        <hr/>        
                        <div class="form-horizontal p-t-20" action="" method="POST" novalidate enctype="multipart/form-data">
                            <div class="form-group row">
                                <label  class="col-sm-3 control-label"><span class="text-danger">*</span>Notes/Comments <br>سبب الإجازة</label>
                                <div class="col-sm-9">
                                    <div class="controls">
                                        <div class="input-group">
                                            <input type="hidden" name="shLId" value="'.$id.'" />
                                            <textarea name="notes" class="form-control notesComments" rows="2"></textarea>
                                            <div class="input-group-prepend">
                                                <span class="input-group-text" id="basic-addon2"><i class="far fa-comment"></i></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                ';
        }        
                
        $message = json_encode(array(
            'id' => $id
            ,'requestNo' => $requestNo    
            ,'message' => $table
            ,'status' => $currentStatus
            ,'error' => 0
        ));
    } else {
        $table = '';
        $table .= '
            <div class="table-responsive">
            <table class="table-bordered" cellspacing="2" cellpadding="10" width="100%">
                <thead>
                    <tr>
                        <th>Initiator</th>
                        <th>Notes/Comments</th>
                        <th>Status</th>
                        <th>Date & Time</th>
                    </tr>
                </thead>
                <tbody>
                    <tr><td colspan="4">No history found!</td></tr>
                </tbody>
            </table>
            </div>';
        $message = json_encode(array(
            'message' => $table
            ,'error' => 0
        ));
    }
    echo $message;     
?>