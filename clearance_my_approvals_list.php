                                                ...
                                                <table id="dynamicTable" class="display table-hover table-striped table-bordered"
                                                    ...
                                                    <tbody>
                                                        <?php
                                                            ...
                                                            $sqlNew = "SELECT cas.*, cp.name as process_name, c.requestNo, c.dateCreated, c.isCleared,
                                                            concat(s.firstName,' ',s.secondName,' ',s.thirdName,' ',s.lastName) as staffName, d.name as department
                                                            FROM clearance_approval_status as cas
                                                            INNER JOIN clearance as c ON cas.clearance_id = c.id
                                                            INNER JOIN staff as s ON cas.staffId = s.staffId
                                                            INNER JOIN employmentdetail as e ON c.staffId = e.staff_id
                                                            INNER JOIN department as d ON d.id = e.department_id
                                                            INNER JOIN clearance_process as cp ON cas.clearance_process_id = cp.id
                                                            WHERE cas.approverStaffId = '$staffId' AND current_flag = 1
                                                            GROUP BY cas.id, cas.clearance_id, cas.clearance_process_id, cas.staffId, cas.approverStaffId,
                                                            cas.status, cas.sequence_no, cas.comment, cas.dateUpdated, cas.current_flag,
                                                            cp.name, c.requestNo, c.dateCreated, c.isCleared, s.firstName,s.secondName,s.thirdName, s.lastName, d.name";
                                                            $rows = $helper->readData($sqlNew);
