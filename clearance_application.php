                                                                ...
                                                                <?php 
                                                                    if(isset($_POST['submit'])) {                                                    
                                                                        ...
                                                                        if($save->insert("exit_interview",$fields)) {                                                                            
                                                                            ...
                                                                            $rows = $helper->readData("SELECT app.position_id, e.staff_id as approverStaffId FROM approvalsequence_standardleave as app LEFT OUTER JOIN employmentdetail as e ON app.approver_id = e.position_id WHERE app.position_id = $myPositionId AND app.active = 1 AND e.isCurrent = 1 AND e.status_id = 1 AND app.sequence_no <= 2 ORDER BY app.sequence_no"); //Getting HoS and HoD from standard leave approval process
                                                            ...
                                                            <div class="card-header bg-light-blue">
                                                                <?php 
                                                                    if(isset($_POST['submit2'])) {                                                    
                                                                        ...
                                                                        $lastId = $getIdInfo->singleReadFullQry("SELECT TOP 1 id FROM clearance ORDER BY id DESC");
                                                                        $clearance_id = $lastId['id'];
                                                                        $rows = $helper->readData("SELECT app.position_id, e.staff_id as approverStaffId FROM approvalsequence_standardleave as app LEFT OUTER JOIN employmentdetail as e ON app.approver_id = e.position_id WHERE app.position_id = $myPositionId AND app.active = 1 AND e.isCurrent = 1 AND e.status_id = 1 AND app.sequence_no <= 2 ORDER BY app.sequence_no");
                                                                        $clearanceProcesses = array();
                                                                        $to = array();
                                                                        if($helper->totalCount != 0) {
                                                                            if (count($rows) == 2) { //1. HoS > 2. HoD
