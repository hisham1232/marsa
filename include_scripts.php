            ...
            $.ajax({
				...
				,success : function(e){
                    if(e.error == 1){
                        ...
                        $.each(e.rows, function(i, j){
                            $('#draftAddOthers tbody').append("<tr><td>" + j.id + "</td><td>" + j.requestNo + "</td><td>" + j.staff_id + "</td><td>" + j.staffName + "</td><td>" + j.fStartDate + "</td><td>" + j.fEndDate + "</td><td>" + j.total + "</td><td><button onClick=\"deleteAddedOthers('"+j.id+"','"+j.requestNo+"')\" type='button' class='btn btn-outline-danger waves-effect waves-light' title='Remove this entry'><i class='fa fa-trash'></i></button></td></tr>");
                        });
                        $('.actionNotification').html(e.notification);
