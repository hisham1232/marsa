<?php    
    ...
    if(!$helper->isUserLogged()) {
        ...
        //$isApproverRow = $isApprover->singleReadFullQry("SELECT TOP 1 * FROM clearance_approval_status WHERE clearance_id = $id AND approverStaffId = '$staffId' AND current_flag = 1 ORDER BY id DESC");
        $isApproverRow = $isApprover->singleReadFullQry("SELECT TOP 1 cas.*, cp.name as roleName FROM clearance_approval_status as cas LEFT OUTER JOIN clearance_process as cp ON cas.clearance_process_id = cp.id WHERE clearance_id = $id AND approverStaffId = '$staffId' AND current_flag = 1 ORDER BY id DESC");
        ...
        if($allowed){  
            ...
            $roleName = $isApproverRow['roleName'];
            ?>
                                            ...
                                            <div class="card-body">
                                                <?php 
                                                    if(isset($_POST['submit'])) {
                                                        ...
                                                        $whichFlag = new DbaseManipulation;
                                                        $row = $whichFlag->singleReadFullQry("SELECT TOP 1 * FROM clearance_approval_status WHERE current_flag = 1 AND approverStaffId = '$approverStaffId' AND clearance_process_id = $processId AND clearance_id = $clearanceId");
                                                        $idOfClearanceApprovalStatus = $row['id'];
                                                        $nextSequence = $row['sequence_no'] + 1;

                                                        $dateUpdated = date('Y-m-d H:i:s');
                                                        $executeQuery = new DbaseManipulation;
                                                        $sql = "UPDATE clearance_approval_status SET current_flag = 0, status = '$sts', comment = '$notes', dateUpdated = '$dateUpdated' WHERE id = $idOfClearanceApprovalStatus";
                                                        $executeQuery->executeSQL($sql);
                                                        $sqlNext = "UPDATE clearance_approval_status SET current_flag = 1 WHERE sequence_no = $nextSequence AND clearance_id = $clearanceId";
                                                        $executeQuery->executeSQL($sqlNext);

                                                        $dateUpdated = date('Y-m-d H:i:s', time());
                                                        $staffIdFiler = $row['staffId'];
                                                        $sqlHistory = "INSERT INTO clearance_history (clearance_id, clearance_process_id, staffId, approverStaffId, status, comment, dateUpdated) VALUES ($clearanceId, $processId, '$staffIdFiler', '$approverStaffId', '$sts', '$notes', '$dateUpdated')";
                                                        $executeQuery->executeSQL($sqlHistory);

                                                        $checkIfCleared = new DbaseManipulation;
                                                        $rowCleared = $checkIfCleared->singleReadFullQry("SELECT COUNT(status) as sts FROM clearance_approval_status WHERE status = 'Completed'");
                                                        if($rowCleared['sts'] == 10) {
                                                            $isClearedNow = new DbaseManipulation;
                                                            $isClearedNow->executeSQL("UPDATE clearance SET isCleared = 1 WHERE id = $clearanceId");
                                                            $clearanceStatus = 'Completed [Cleared]';
                                                        } else {
                                                            $clearanceStatus = 'Pending [On Process]';
                                                        }
                                                        //echo $sql.'<br>';
                                                        //echo $sqlNext.'<br>';
                                                        //echo $sqlHistory.'<br>';
                                                        $to = array();
                                                        $getEmailInfo = new DbaseManipulation;
                                                        $staffEmail = $getEmailInfo->getContactInfo(2,$staffIdFiler,'data');
                                                        $approverEmail = $getEmailInfo->getContactInfo(2,$approverStaffId,'data');
                                                        array_push($to,$staffEmail,$approverEmail);
                                                        $nextApprover = new DbaseManipulation;
                                                        $rows = $nextApprover->readData("SELECT approverStaffId FROM clearance_approval_status WHERE current_flag = 1");
                                                        if($nextApprover->totalCount != 0) {
                                                            foreach ($rows as $row) {
                                                                array_push($to,$getEmailInfo->getContactInfo(2,$row['approverStaffId'],'data'));
                                                            }
                                                        }
                                                        ...
                                                        WHERE cas.current_flag = 1 AND clearance_id = $clearanceId");
                                                        $cycle_status = 'For Approval of '.$row['currProcess'].' - '.$row['approverName'];
                                                        $gsm = $helper->getContactInfo(1,$staffIdFiler,'data');

                                                        $clearanceInfo = new DbaseManipulation;
                                                        $rowCI = $clearanceInfo->singleReadFullQry("SELECT TOP 1 * FROM clearance WHERE id = $clearanceId");
                                                        $requestNo = $rowCI['requestNo'];
                                                        $requestDate = $rowCI['dateCreated'];

                                                        $from_name = 'hrms@nct.edu.om';
                                                        $from = 'HRMS - 3.0';
                                                        $subject = 'NCT-HRMD CLEARANCE APPLICATION FILED BY '.strtoupper($helper->getStaffName($staffIdFiler,'firstName','secondName','thirdName','lastName'));
                                                        $message = '<html><body>';
                                                        ...
                                                        $message .= "<tr style='background:#E0F8F7; width:400px'><td><strong>CLEARANCE STATUS:</strong> </td><td>".$clearanceStatus."</td></tr>";
                                                        ...
                                                        $message .= "<tr style='background:#E0F8F7'><td><strong>DATE FILED:</strong> </td><td>".date('d/m/Y H:i:s',strtotime($requestDate))."</td></tr>";
                                                        $message .= "<tr style='background:#EFFBFB'><td><strong>STAFF ID:</strong> </td><td>".$staffIdFiler."</td></tr>";
                                                        $message .= "<tr style='background:#E0F8F7'><td><strong>STAFF NAME:</strong> </td><td>".$helper->getStaffName($staffIdFiler,'firstName','secondName','thirdName','lastName')."</td></tr>";
                                                        $message .= "<tr style='background:#EFFBFB'><td><strong>STAFF DEPARTMENT:</strong> </td><td>".$info['department']."</td></tr>";
                                                        $message .= "<tr style='background:#E0F8F7'><td><strong>EMAIL ADDRESS:</strong> </td><td>".$staffEmail."</td></tr>";
                                                        $message .= "<tr style='background:#EFFBFB'><td><strong>MOBILE NUMBER:</strong> </td><td>".$gsm."</td></tr>";
                                                        $message .= "</table>";
                                                        $message .= "<hr/>";
                                                        $message .= "<h3>NCT-HRMS 3.0 CLEARANCE APPLICATION HISTORIES</h3>";
                                                        $message .= '<table style="border-color:#666; width:800px" cellpadding="10">';
                                                        ...
                                                        $message .= "</tr>";
                                                        $ctr = 1; $stripeColor = "";    
                                                        $hist = new DbaseManipulation;
                                                        $history = $hist->readData("SELECT * FROM clearance_history WHERE clearance_id = $clearanceId ORDER BY id DESC");
                                                        foreach($history as $row){
                                                            ...
                                                            $fullStaffNameEmail = $helper->getStaffName($row['approverStaffId'],'firstName','secondName','thirdName','lastName');
                                                            $dateNotesEmail = date('d/m/Y H:i:s',strtotime($row['dateUpdated']));
                                                            $notesEmail = $row['comment'];
                                                                                ...
                                                                                <div class="col-sm-9">
                                                                                    <div class="controls">
                                                                                        <div class="input-group">
                                                                                            <input type="text" class="form-control" readonly value="<?php echo $roleName; ?>">
