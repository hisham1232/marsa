...
class Db {
	protected function connect() {
		...
		$password = "15935789";
		try {
			$conn = new PDO("mysql:host=$servername;dbname=dbhr3_test;charset=utf8", $username, $password);
...
class DbaseManipulation extends Db { //Inheritance, inheriting class Db
	...
	public function requestNo($prefix,$tableName) {
		try {
			global $lastRow;
			$sql = "SELECT requestNo FROM ".$tableName." ORDER BY id DESC LIMIT 1";
			$stmt = $this->connect()->prepare($sql);
			$stmt->execute();
			if ($stmt->rowCount() > 0) {
	...
	public function internalLeaveBalanceId($prefix,$tableName) {
		try {
			global $lastRow;
			$sql = "SELECT internalleavebalance_id FROM ".$tableName." ORDER BY id DESC LIMIT 1";
			$stmt = $this->connect()->prepare($sql);
			$stmt->execute();
			if ($stmt->rowCount() > 0) {
				$row = $stmt->fetch(PDO::FETCH_ASSOC);
	...
	public function emergencyLeaveBalanceId($prefix,$tableName) {
		try {
			global $lastRow;
			$sql = "SELECT emergencyleavebalance_id FROM ".$tableName." ORDER BY id DESC LIMIT 1";
			$stmt = $this->connect()->prepare($sql);
			$stmt->execute();
			if ($stmt->rowCount() > 0) {
				$row = $stmt->fetch(PDO::FETCH_ASSOC);
	...
	public function shortLeaveHistory($id,$fieldname) {
		try {
			$sql = "SELECT * FROM shortleave_history WHERE shortleave_id = :id ORDER BY id DESC LIMIT 1";
			$stmt = $this->connect()->prepare($sql);
			$stmt->bindValue(':id',$id);
			$stmt->execute();
			$result = $stmt->fetch(PDO::FETCH_ASSOC);
	...
	public function standardLeaveHistory($id,$fieldname) {
		try {
			$sql = "SELECT * FROM standardleave_history WHERE standardleave_id = :id ORDER BY id DESC LIMIT 1";
			$stmt = $this->connect()->prepare($sql);
			$stmt->bindValue(':id',$id);
			$stmt->execute();
			$result = $stmt->fetch(PDO::FETCH_ASSOC);
	...
	public function overtimeLastStatus($requestNo,$fieldname) {
		try {
			$sql = "SELECT * FROM internalleaveovertime_history WHERE requestNo = :requestNo ORDER BY id DESC LIMIT 1";
			$stmt = $this->connect()->prepare($sql);
	...
	public function get_staff_id_position_id($position_id,$fieldname) {
		try {
			$sql = "SELECT staff_id FROM employmentdetail WHERE position_id = :position_id AND isCurrent = 1 LIMIT 1";
			$stmt = $this->connect()->prepare($sql);
			$stmt->bindValue(':position_id',$position_id);
			$stmt->execute();
			$result = $stmt->fetch(PDO::FETCH_ASSOC);
