<?php     
    ...
    if(!$helper->isUserLogged()) {
        ...
        $allowed = ($helper->isAllowed('SuperAdmin') || $helper->isAllowed('HRHead') || $helper->isAllowed('HoS')|| $helper->isAllowed('HoD_HoC')) ? true : false;
        //$allowed =  true;
                                    ...
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            <table class="display nowrap table table-hover table-striped table-bordered">
                                                <?php 
                                                    $stf = $helper->singleReadFullQry("SELECT COUNT(id) as staffTotal FROM employmentdetail WHERE status_id = 1 AND isCurrent = 1 AND section_id = $logged_in_section_id");
                                                    $totalStaff = $stf['staffTotal'];
                                                    $ctrTech = $helper->singleReadFullQry("SELECT COUNT(id) as ctr1 FROM appraisal_technician WHERE appraisal_year = '$appraisalYear' AND section_id = $logged_in_section_id");
                                                    $submitted = $submitted + $ctrTech['ctr1'];
                                                    $ctrLect = $helper->singleReadFullQry("SELECT COUNT(id) as ctr2 FROM appraisal_lecturer WHERE appraisal_year = '$appraisalYear' AND section_id = $logged_in_section_id");
                                                    $submitted = $submitted + $ctrLect['ctr2'];
                                                    $ctrAdm = $helper->singleReadFullQry("SELECT COUNT(id) as ctr3 FROM appraisal_admin WHERE appraisal_year = '$appraisalYear' AND section_id = $logged_in_section_id");
                                                    $submitted = $submitted + $ctrAdm['ctr3'];
                                                    //$ctrHoS = $helper->singleReadFullQry("SELECT COUNT(id) as ctr4 FROM appraisal_hos WHERE appraisal_year = '$appraisalYear' AND section_id = $logged_in_section_id");
                                                    //$submitted = $submitted + $ctrHoS['ctr4'];
                                                    $ctrHoD = $helper->singleReadFullQry("SELECT COUNT(id) as ctr5 FROM appraisal_hod WHERE appraisal_year = '$appraisalYear' AND section_id = $logged_in_section_id");
                                                    $submitted = $submitted + $ctrHoD['ctr5'];

                                                    $submittedCompleted = 0; 
                                                    $ctrTechCompleted = $helper->singleReadFullQry("SELECT COUNT(id) as ctr1 FROM appraisal_technician WHERE appraisal_year = '$appraisalYear' AND status = 'Completed' AND section_id = $logged_in_section_id");
                                                    $submittedCompleted = $submittedCompleted + $ctrTechCompleted['ctr1'];
                                                    $ctrLectCompleted = $helper->singleReadFullQry("SELECT COUNT(id) as ctr2 FROM appraisal_lecturer WHERE appraisal_year = '$appraisalYear' AND status = 'Completed' AND section_id = $logged_in_section_id");
                                                    $submittedCompleted = $submittedCompleted + $ctrLectCompleted['ctr2'];
                                                    $ctrAdmCompleted = $helper->singleReadFullQry("SELECT COUNT(id) as ctr3 FROM appraisal_admin WHERE appraisal_year = '$appraisalYear' AND status = 'Completed' AND section_id = $logged_in_section_id");
                                                    $submittedCompleted = $submittedCompleted + $ctrAdmCompleted['ctr3'];
                                                    $ctrHoSCompleted = $helper->singleReadFullQry("SELECT COUNT(id) as ctr4 FROM appraisal_hos WHERE appraisal_year = '$appraisalYear' AND status = 'Completed' AND section_id = $logged_in_section_id");
                                                    $submittedCompleted = $submittedCompleted + $ctrHoSCompleted['ctr4'];
                                                    $ctrHoDCompleted = $helper->singleReadFullQry("SELECT COUNT(id) as ctr5 FROM appraisal_hod WHERE appraisal_year = '$appraisalYear' AND status = 'Completed' AND section_id = $logged_in_section_id");
                                                            ...
                                                            <tbody>
                                                                <?php 
                                                                    ...
                                                                    foreach ($rowsTech as $rowTech) {
                                                                        ...
                                                                        <tr>
                                                                            <td>
                                                                                <a href="appraisal_technician_hos_view.php?id=<?php echo $rowTech['id']; ?>" title="Click to View Appraisal Details"> <span class="text-primary font-weight-bold"><?php echo $rowTech['staffName']; ?></span></a>
                                                                            </td>
                                                                ...
                                                                ?>
                                                                    ...
                                                                    foreach ($rowsAdm as $rowAdm) {
                                                                        ...
                                                                        <tr>
                                                                            <td>
                                                                                <a href="appraisal_adminstaff_hod_view.php?id=<?php echo $rowAdm['id']; ?>" title="Click to View Appraisal Details"> <span class="text-primary font-weight-bold"><?php echo $rowAdm['staffName']; ?></span></a>
                                                                            </td>
                                                                ...
                                                                <?php 
                                                                    ...
                                                                    foreach ($rowsLect as $rowLect) {
                                                                        ...
                                                                        <tr>
                                                                            <td>
                                                                                <a href="appraisal_lecturer_hos_view.php?id=<?php echo $rowLect['id']; ?>" title="Click to View Appraisal Details"> <span class="text-primary font-weight-bold"><?php echo $rowLect['staffName']; ?></span></a>
                                                                            </td>
