                                                            ...
                                                            <div class="card-header bg-light-blue">
                                                                <?php 
                                                                    if(isset($_POST['submit2'])) {                                                    
                                                                        ...
                                                                        $rows = $helper->readData("SELECT ca.position_id, ca.sequence_no, e.staff_id as approverStaffId FROM clearance_approver as ca INNER JOIN employmentdetail as e ON ca.position_id = e.position_id WHERE ca.active = 1 AND e.isCurrent = 1 AND e.status_id = 1 ORDER BY ca.id");
                                                                        foreach ($rows as $row) {
                                                                            $approverStaffId = $row['approverStaffId'];
                                                                            $sequence_no = $row['sequence_no'];
                                                                            $sql = "($clearance_id, $ctr, '$staffId', $approverStaffId, $sequence_no, 'Pending', 'N/A', '$now_date', 0)";
