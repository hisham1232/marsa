                                        ...
                                        <div class="card-body">
                                            <div class="table-responsive">
                                                <table id="dynamicTable" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                                    ...
                                                    <tbody>
                                                        <?php
                                                            $rows = $helper->readData("SELECT ot.*, concat(s.firstName,' ',s.secondName,' ',s.thirdName,' ',s.lastName) as requestor, d.name as department FROM internalleaveovertimefiled as ot LEFT OUTER JOIN staff as s ON s.staffId = ot.createdBy LEFT OUTER JOIN employmentdetail as e ON e.staff_id = ot.createdBy LEFT OUTER JOIN department as d ON d.id = e.department_id WHERE e.isCurrent = 1 and e.status_id = 1 ORDER BY ot.id DESC");
