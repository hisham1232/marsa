                        ...
                        <div class="container-fluid">
                            <div class="row page-titles">
                                <div class="col-md-5 col-8 align-self-center">
                                    <h3 class="text-themecolor m-b-0 m-t-0">Staff Short Leave List</h3>
                                    <ol class="breadcrumb">
                                        <li class="breadcrumb-item">Home</a></li>
                                        <li class="breadcrumb-item">My Department/Section</li>
                                        <li class="breadcrumb-item">My Staff List</li>
                                        <li class="breadcrumb-item active">Staff Short Leave</li>
                                    </ol>
                                </div>
                                                    ...
                                                    <tbody>
                                                        <?php
                                                            if(isset($_POST['searchByDate'])) {
                                                                ...
                                                                LEFT OUTER JOIN department as d ON d.id = e.department_id
                                                                LEFT OUTER JOIN sponsor as sp ON sp.id = e.sponsor_id
                                                                WHERE sh.leaveDate >= '$startDate' AND sh.leaveDate <= '$endDate' AND e.isCurrent = 1 AND sh.staff_id = $id ORDER BY sh.id DESC";
                                                            } else {
                                                                $SQL = "SELECT sh.*, d.name as department, concat(s.firstName,' ',s.secondName,' ',s.thirdName,' ',s.lastName) as staffName, sp.name as sponsor FROM shortleave as sh 
                                                                LEFT OUTER JOIN employmentdetail as e ON e.staff_id = sh.staff_id
                                                                LEFT OUTER JOIN staff as s ON s.staffId = sh.staff_id
                                                                LEFT OUTER JOIN department as d ON d.id = e.department_id
                                                                LEFT OUTER JOIN sponsor as sp ON sp.id = e.sponsor_id
                                                                WHERE e.isCurrent = 1 AND sh.staff_id = $id
                                                                ORDER BY sh.id DESC";
                                                            }
                                                            //echo $SQL;
